...
-- The original license-limit migration was merged with a timestamp older than
-- the latest migration already applied in production, so the 27-argument RPC
-- never replaced the 25-argument provisioning function there. Repair the
-- contract using a forward migration while remaining safe in environments
-- where the original migration was applied successfully.
do $$
begin
    if to_regprocedure(
        'landlord.create_new_tenant_without_license_limits(text,text,text,text,boolean,text,text,text,uuid,uuid,text,text,boolean,boolean,text,text,text,text,boolean,boolean,boolean,boolean,boolean,text,text)'
    ) is null then
        if to_regprocedure(
            'landlord.create_new_tenant(text,text,text,text,boolean,text,text,text,uuid,uuid,text,text,boolean,boolean,text,text,text,text,boolean,boolean,boolean,boolean,boolean,text,text)'
        ) is null then
            raise exception 'No se encontró la función base landlord.create_new_tenant esperada.';
        end if;

        execute $sql$
            alter function landlord.create_new_tenant(
                text, text, text, text, boolean, text, text, text, uuid, uuid,
                text, text, boolean, boolean, text, text, text, text, boolean,
...
revoke all on function landlord.create_new_tenant(
    text, text, text, text, boolean, text, text, text, uuid, uuid,
    text, text, boolean, boolean, text, text, text, text, boolean,
    boolean, boolean, boolean, boolean, text, text, integer, integer
) from public, anon, authenticated;
