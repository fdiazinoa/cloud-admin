...
-- Extend auth_status to surface license exhaustion to POS / Cloud-Admin.
...
DROP FUNCTION IF EXISTS public.enforce_tenant_pos_license_limits(UUID);

CREATE FUNCTION public.enforce_tenant_pos_license_limits(p_tenant_id UUID)
RETURNS JSONB
LANGUAGE sql
SECURITY DEFINER
SET search_path = public, landlord
AS $$
    SELECT landlord.enforce_tenant_pos_license_limits(p_tenant_id);
$$;

DROP FUNCTION IF EXISTS public.resolve_tenant_license(UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.resolve_tenant_license(UUID, TEXT, TEXT, TEXT);
...
BEGIN
    ...
    SELECT COUNT(DISTINCT registry.terminal_id)
    INTO v_active_terminals
    ...
    IF v_device_id IS NOT NULL THEN
        ...
        ELSIF v_device_auth_status IS NULL THEN
            SELECT COUNT(DISTINCT registry.terminal_id) >= v_limit
            INTO v_device_allowed
            FROM landlord.tenant_server_registry AS registry
            WHERE registry.tenant_id = v_tenant.id
              AND registry.status = 'ONLINE'
              AND COALESCE(registry.is_revoked, FALSE) = FALSE
...
DECLARE
    ...
    v_limit INTEGER;
    v_active_terminals INTEGER;
    v_terminal_already_licensed BOOLEAN;
...
BEGIN
    ...
    SELECT EXISTS (
        ...
        FROM landlord.tenant_server_registry AS registry
        WHERE registry.tenant_id = v_tenant.id
          AND registry.terminal_id = p_terminal_id
          AND registry.status = 'ONLINE'
          AND COALESCE(registry.is_revoked, FALSE) = FALSE
          AND COALESCE(registry.auth_status, 'AUTHORIZED') <> 'LICENSE_EXCEEDED'
    ) INTO v_terminal_already_licensed;

    IF NOT v_device_exists AND NOT v_terminal_already_licensed THEN
        SELECT COUNT(DISTINCT registry.terminal_id)
        INTO v_active_terminals
        FROM landlord.tenant_server_registry AS registry
        WHERE registry.tenant_id = v_tenant.id
        ...
        IF v_active_terminals >= v_limit THEN
            RAISE EXCEPTION
                'POS_LICENSE_LIMIT_EXCEEDED: sin licencias POS contratadas (%/%). Contrata mas terminales.',
                v_active_terminals,
                v_limit
                USING ERRCODE = 'P0001';
        END IF;
...
REVOKE ALL ON FUNCTION landlord.enforce_tenant_pos_license_limits(UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.enforce_tenant_pos_license_limits(UUID) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION landlord.enforce_tenant_pos_license_limits(UUID) TO service_role;
GRANT EXECUTE ON FUNCTION public.enforce_tenant_pos_license_limits(UUID) TO service_role, authenticated;

REVOKE ALL ON FUNCTION public.resolve_tenant_license(UUID, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.resolve_tenant_license(UUID, TEXT, TEXT, TEXT) TO anon, authenticated;
