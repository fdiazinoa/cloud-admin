...
-- Currency history remains append-only for every ordinary UPDATE/DELETE.
-- A transaction-local list permits only the ERP tenant IDs already validated
-- by landlord.delete_tenant after exact-name confirmation.
CREATE OR REPLACE FUNCTION private.erp_currency_history_append_only()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
DECLARE
    v_allowed_tenant_ids TEXT[] := string_to_array(
        NULLIF(current_setting('landlord.tenant_delete_erp_tenant_ids', TRUE), ''),
        ','
    );
BEGIN
    IF tg_op = 'DELETE'
       AND old.tenant_id::TEXT = ANY(COALESCE(v_allowed_tenant_ids, ARRAY[]::TEXT[]))
    THEN
        RETURN old;
    END IF;

    RAISE EXCEPTION 'erp_currency_rate_history is append-only'
        USING ERRCODE = '55000';
END;
$$;

-- Tenant deletion is an administrative operation that can legitimately exceed
-- PostgREST's default eight-second statement timeout. Keep a bounded function-
-- local timeout and preserve tenant_id indexes in the dynamic purge.
...
BEGIN
    ...
    IF COALESCE(array_length(v_erp_tenant_ids, 1), 0) > 0 THEN
        PERFORM set_config(
            'landlord.tenant_delete_erp_tenant_ids',
            array_to_string(v_erp_tenant_ids, ','),
            TRUE
        );

        DELETE FROM public.erp_sync_domain_versions
        ...
        LOOP
            IF v_table.tenant_id_type_oid = 'uuid'::REGTYPE THEN
                EXECUTE format(
                    'DELETE FROM %I.%I WHERE tenant_id = ANY($1)',
                    v_table.schema_name,
                    v_table.table_name
                )
                USING v_erp_tenant_ids;
            ELSE
                EXECUTE format(
        ...
        PERFORM set_config('landlord.tenant_delete_erp_tenant_ids', '', TRUE);
    END IF;
