...
type DeviceAction = 'TAKEOVER' | 'ROTATE_TOKEN' | 'REVOKE_DEVICE' | 'GENERATE_PAIRING_CODE';
...
Deno.serve(async (request) => {
    ...
    try {
        ...
        if (!['TAKEOVER', 'ROTATE_TOKEN', 'REVOKE_DEVICE', 'GENERATE_PAIRING_CODE'].includes(action)) {
        ...
        if (action === 'GENERATE_PAIRING_CODE') {
            ...
            if (!erpResponse.ok) {
                await insertDeviceAudit(supabase, {
                    ...
                    terminal_id: terminalId,
                    terminal_name: terminalName || registry?.terminal_name || publicTerminal?.name || null,
                    old_device_id: authorizedDeviceId,
